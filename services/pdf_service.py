...
def generate_pdf_from_data(data: dict) -> bytes:
    ...
    # Center the line based on title width (approximate)
    ...
    for sentiment, value in sentiments.items():
        ...
        if isinstance(value, (int, float)):
            bar_len = (value / max_val) * 100  # Scale to 100mm max
    ...
    for topic in topics:
        ...
        # Print Count aligned right (using trick: print right aligned first, then reset x)
        # Actually, simpler: cell(0) align R, reset x, cell(0) align L? 
        # Or just calculate x for count?
        # Let's use the overlay method
